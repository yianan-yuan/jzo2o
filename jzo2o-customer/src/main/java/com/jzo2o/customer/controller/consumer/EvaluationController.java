...
public class EvaluationController {
    ...
    @PostMapping
    @ApiOperation("发表评价")
    public BooleanResDTO submit(@RequestBody UnifiedEvaluationSubmitReqDTO evaluationSubmitReqDTO) {
        return unifiedEvaluationService.submit(UserContext.currentUser().getId(), evaluationSubmitReqDTO);
    }

    @GetMapping("/page")
    @ApiOperation("分页查询当前客户的评价")
    public PageResult<UnifiedEvaluationResDTO> page(UnifiedEvaluationPageReqDTO req) {
        return unifiedEvaluationService.pageForConsumer(UserContext.currentUser().getId(), req);
    }

    @GetMapping("/{id}")
    @ApiOperation("查询当前客户的评价详情")
    public UnifiedEvaluationResDTO detail(@PathVariable("id") Long id) {
        return unifiedEvaluationService.detailForConsumer(id, UserContext.currentUser().getId());
    }
    ...
    public List<EvaluationResDTO> pageByTargetId(EvaluationPageByTargetReqDTO evaluationPageByTargetReqDTO) {
        return new ArrayList<EvaluationResDTO>();
