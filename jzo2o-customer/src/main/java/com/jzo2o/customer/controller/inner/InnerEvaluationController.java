...
public class InnerEvaluationController implements EvaluationApi {
    ...
    @ApiOperation("批量查询服务人员好评率")
    public Map<Long, ServeProviderGoodRateResDTO> queryServeProviderGoodRates(
            @RequestParam("serveProviderIds") List<Long> serveProviderIds) {
        return unifiedEvaluationService.rateForWorkers(serveProviderIds).entrySet().stream()
                .collect(Collectors.toMap(Map.Entry::getKey,
                        entry -> new ServeProviderGoodRateResDTO(
                                entry.getValue().getHasEvaluation(), entry.getValue().getGoodRate())));
    }
