...
public class EvaluationServiceImpl implements EvaluationService {
    ...
    public BooleanResDTO submit(EvaluationSubmitReqDTO evaluationSubmitReqDTO) {
        ...
        if (ObjectUtil.notEqual(orderResDTO.getOrdersStatus(), 400)) {
            // 非待评价状态也允许重新评价（之前已经评过，重复提交）
            log.warn("订单当前状态不是待评价(400)，当前状态：{}，订单id：{}", orderResDTO.getOrdersStatus(), evaluationSubmitReqDTO.getOrdersId());
        }

        CurrentUserInfo currentUserInfo = evaluationSubmitReqDTO.getCurrentUserInfo();

        // ========== 本地开发模式：跳过外部评价系统调用 ==========
        // 当前没有外部评价系统，注释掉 4 次外部 HTTP 调用（2 次 query + 2 次 submit），
        // 仅保留订单状态机变更。评价内容（文字/图片/打分）暂不持久化。
        // 如果以后接回了外部评价系统，把下面的注释代码取消注释即可。
        ...
        */
        ...
        //根据订单id评价（触发订单状态机变更：NO_EVALUATION -> FINISHED）
        log.info("开发模式：跳过外部评价系统，仅更新订单状态。订单id：{}", evaluationSubmitReqDTO.getOrdersId());
        // 如果状态机变更失败（比如订单状态不是400），不阻塞主流程
        try {
            ordersApi.evaluate(evaluationSubmitReqDTO.getOrdersId());
        } catch (Exception e) {
            log.error("触发订单状态机评价事件失败，订单id：{}", evaluationSubmitReqDTO.getOrdersId(), e);
        }
    ...
    private void requestSubmitServeItemEvaluation(EvaluationSubmitReqDTO evaluationSubmitReqDTO, CurrentUserInfo currentUserInfo) {
        // 如果没有配置外部评价服务地址，跳过调用
        if (ObjectUtil.isEmpty(evaluationProperties.getDomain())) {
            log.warn("jzo2o.evaluation.domain 未配置，跳过服务项评价同步");
            return;
        }
        ...
        try {
            ...
            // 外部评价服务调用失败不影响主流程（订单状态机变更）
            log.error("同步服务项评价到外部评价系统失败", e);
        }
    }
    ...
    private void requestSubmitServeProviderEvaluation(EvaluationSubmitReqDTO evaluationSubmitReqDTO, CurrentUserInfo currentUserInfo) {
        // 如果没有配置外部评价服务地址，跳过调用
        if (ObjectUtil.isEmpty(evaluationProperties.getDomain())) {
            log.warn("jzo2o.evaluation.domain 未配置，跳过服务人员/机构评价同步");
            return;
        }
        ...
        try {
            ...
            // 外部评价服务调用失败不影响主流程（订单状态机变更）
            log.error("同步服务人员/机构评价到外部评价系统失败", e);
        }
    }
    ...
    public EvaluationResDTO queryByTargetTypeIdAndOrdersId(CurrentUserInfo currentUserInfo, String targetTypeId, Long ordersId) {
        // 如果没有配置外部评价服务地址，直接返回 null（视为该订单未评价过）
        if (ObjectUtil.isEmpty(evaluationProperties.getDomain())) {
            return null;
        }
        ...
        try {
            ...
            // 外部评价服务查询失败不影响主流程
            log.error("查询外部评价系统失败，ordersId：{}", ordersId, e);
            return null;
        }
    }
