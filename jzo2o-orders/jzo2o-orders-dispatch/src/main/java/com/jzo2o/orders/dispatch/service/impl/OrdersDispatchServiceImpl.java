...
import com.jzo2o.orders.dispatch.service.GoodRateCandidateEnricher;
...
//    }
    ...
    public void dispatch(Long id) {
        ...
        if (DispatchStrategyEnum.GOOD_RATE.equals(dispatchStrategyEnum)
                || DispatchStrategyEnum.LEAST_ACCEPT_ORDER.equals(dispatchStrategyEnum)) {
            fillGoodRates(serveProvidersOfServe);
        }
...
//        // 4.机器抢单
    ...
    private void fillGoodRates(List<ServeProviderDTO> candidates) {
        try {
            List<Long> providerIds = candidates.stream()
                    .map(ServeProviderDTO::getId)
                    .filter(Objects::nonNull)
                    .collect(Collectors.toList());
            Map<Long, ServeProviderGoodRateResDTO> rates = evaluationApi.queryServeProviderGoodRates(providerIds);
            GoodRateCandidateEnricher.apply(candidates, rates);
        } catch (Exception e) {
            log.warn("Failed to query provider good rates, using unrated fallback", e);
            GoodRateCandidateEnricher.apply(candidates, null);
        }
    }
    ...
    public List<ServeProviderDTO> searchDispatchInfo(String cityCode, long serveItemId, double maxDistance, int serveTime, DispatchStrategyEnum dispatchStrategyEnum, Double lon, Double lat, int limit) {
        ...
        // 5.查询数据
        SearchResponse<ServeProviderDTO> searchResponse = elasticSearchTemplate.opsForDoc().search(build, ServeProviderDTO.class);
        ...
        if (DispatchStrategyEnum.DISTANCE.equals(dispatchStrategyEnum)
                || DispatchStrategyEnum.GOOD_RATE.equals(dispatchStrategyEnum)) {
            ...
            // 6.2.好评率优先和最少接单优先均保留距离，作为最终兜底排序条件
