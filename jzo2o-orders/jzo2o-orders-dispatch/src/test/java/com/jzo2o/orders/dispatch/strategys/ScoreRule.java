 ...
 */
public class ScoreRule implements IProcessRule {
    //下一个节点
    ...
    public List<ServeProviderDTO> doFilter(List<ServeProviderDTO> serveProviderDTOS) {
        ...
        serveProviderDTOS.sort((o1, o2) -> {
            return o2.getEvaluationScore() - o1.getEvaluationScore();
        });

        //取出第一个
        ServeProviderDTO serveProviderDTO = serveProviderDTOS.get(0);
        //最高评分
        Integer evaluationScore = serveProviderDTO.getEvaluationScore();
        //取出距离最优先的人
        List<ServeProviderDTO> collect = serveProviderDTOS.stream().filter(source -> source.getEvaluationScore() == evaluationScore).collect(Collectors.toList());
        return collect;
    }
