...
public class HistoryOrdersServeServiceImpl extends ServiceImpl<HistoryOrdersServeMapper, HistoryOrdersServe> implements IHistoryOrdersServeService {
    ...
    public void migrate() {
        ...
        // 统计迁移数据数量
        Long total = historyOrdersServeSyncService.countBySortTime( yesterDayStartTime,yesterDayEndTime);
        if(total <= 0){
            return;
        }
    ...
    public void deleteMigrated() {
        ...
        // 1.校验是否可以删除已迁移服务单
        // 查询即将删除的数据数量
        Long totalOfDelete = historyOrdersServeSyncService.countBySortTime(yesterDayStartTime, yesterDayEndTime);
        if(totalOfDelete <= 0) {
            log.debug("无迁移服务单数据需要删除");
            return;
        }
        // 查询已经迁移的数量
        Long totalMigrated = lambdaQuery().between(HistoryOrdersServe::getSortTime, yesterDayStartTime, yesterDayEndTime)
                .count();
