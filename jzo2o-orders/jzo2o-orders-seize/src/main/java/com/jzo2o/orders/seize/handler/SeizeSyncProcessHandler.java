...
public class SeizeSyncProcessHandler implements SyncProcessHandler<Object> {
    ...
    public void singleProcess(SyncMessage<Object> singleData) {
        ...
        String valueString = singleData.getValue().toString();
...
//        boolean isMatchine = seizeResult.getBool(2);
        String[] seizeResult = valueString.split(",");
        Long serveProviderId = Long.valueOf(seizeResult[0]);
        Integer serveProviderType = Integer.valueOf(seizeResult[1]);
        boolean isMatchine = Boolean.parseBoolean(seizeResult[2]);
        // 抢单id
