<!-- 评价页面 -->
<template>
  <view class="evaluate workerSettingsWarm">
    <!-- 手机状态栏 -->
...
const getTabIndex = (index) => {
  ...
  if (index === 0) {
    ...
    requestData.value = {
      pageNo: 1,
      pageSize: 10,
      evaluationType: tabBars[index].value,
    };
  }
  homeList.data = [];
  getEvaluateListFunc();
...
const getEvaluateListFunc = () => {
  getEvaluateList(requestData.value).then((res) => {
    ...
    if (body.list) {
      homeList.data = requestData.value.pageNo === 1 ? body.list : [...homeList.data, ...body.list];
      if (homeList.data.length >= Number(body.total || 0)) {
