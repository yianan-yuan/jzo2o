        ...
        >
          ...
          <template #op="{ row }">
            <a
              ...
              "
              :aria-disabled="!canRefund(row)"
              :tabindex="canRefund(row) ? 0 : -1"
...
// 点击退款
const canRefund = (row) => {
  const refundableStatus = [100, 200, 300, 400, 500]
  return row.payStatus === 4 && !row.refundStatus && refundableStatus.includes(row.ordersStatus)
}

const handleClickRefund = (val) => {
  if (!canRefund(val)) return
