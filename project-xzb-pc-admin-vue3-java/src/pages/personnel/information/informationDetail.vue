...
import { getProviderEvaluationSummary } from '@/api/evaluation'
// 引用正则
...
const getEvaluationSummary = async (val) => {
  await getProviderEvaluationSummary(val).then((res) => {
    if (res.code === 200) {
      evaluationSummary.value = res.data
    }
  })
}
// 获取认证信息
...
const serviceData = async () => {
  try {
    const res = await servicePersonData(requestServiceData.value)
    if (res?.code == 200) {
      listData.value = res.data.list
      pagination.total = Number(res.data.total)
    } else {
      MessagePlugin.error(res?.msg || '服务数据加载失败，请稍后重试')
    }
  } catch (error) {
    MessagePlugin.error('服务数据加载失败，请稍后重试')
  }
}
</script>
