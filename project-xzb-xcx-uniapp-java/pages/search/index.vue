...
const refreshSearchCity = () => {
  ...
  searchData.value.cityCode = cityCode;
  return true;
};
...
// ------定义方法------
onShow(() => {
  const storedHistory = uni.getStorageSync('historyData');
  searchHistoryData.value = Array.isArray(storedHistory)
    ? storedHistory.slice(0, 10)
    : [];
});
// 获取列表
const getNewData = async () => {
  if (!searchData.value.cityCode) return;
  ...
  await getServeSearch(searchData.value)
    .then((res) => {
      if (res.data.code == 200) {
        const { data } = res.data;
        // 下拉数据合并
        itemData.value = Array.isArray(data) ? data : [];
...
// 搜索
const handleSearch = () => {
  const keyword = searchData.value.keyword.trim();
  searchData.value.keyword = keyword;
  if (!keyword) {
    handleClear();
    return;
  }
  if (!refreshSearchCity()) return;
  itemData.value = [];
  getNewData();
...
const handleInput = (e) => {
  ...
  searchData.value.keyword = keyword;
  showClearIcon.value = Boolean(keyword);
  if (!keyword) {
    itemData.value = [];
